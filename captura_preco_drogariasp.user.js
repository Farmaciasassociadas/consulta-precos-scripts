...
// @match        https://www.drogariasaopaulo.com.br/*
...
(function () {
    ...
    const SITE = 'drogariasp';
    // BLOQUEIO ANTIBOT (07/2026): Drogaria São Paulo consulta via API/fetch (não navega
    ...
    // O site da Drogaria São Paulo roda em VTEX, que expõe uma API pública de catálogo.
    ...
    // "Principio Ativo": a Drogaria São Paulo e o site que mais expoe isso de forma
    ...
    // A Drogaria São Paulo guarda o EAN EXATAMENTE como cadastrado (com zeros à
    ...
    // A Drogaria São Paulo é VTEX com REGIONALIZAÇÃO: parte do catálogo (ex.: genéricos
    ...
    async function consultar() {
        ...
        if (!produto) {
            for (const termo of variacoesDoEan(ean)) {
                ...
                if (certo) {
                    produto = certo;
                    console.log('[assistente-ean] Drogaria São Paulo: achado pela Intelligent Search (regional)');
                    break;
                }
            }
        }
        ...
        if (!produto) {
            ...
            for (const l of links.slice(0, 3)) {
                ...
                if (eanBate(p)) {
                    p.urlPagina = l.url;
                    produto = p;
                    console.log('[assistente-ean] Drogaria São Paulo: achado pela PAGINA de busca (fallback DOM)');
                    break;
                }
            }
        }
    ...
    async function consultarPorNome(ean) {
        ...
        if (melhor && melhorNota >= LIMIAR_NOME) {
            console.log('[assistente-ean] Drogaria São Paulo: candidato por nome aceito (nota', melhorNota.toFixed(2), ')');
        ...
        if (melhorLink && notaLink >= LIMIAR_NOME) {
            ...
            if (produto) {
                ...
                console.log('[assistente-ean] Drogaria São Paulo: candidato por nome via PAGINA (nota', notaLink.toFixed(2), ')');
    ...
    function emitirDeProduto(produto, ean, porNome) {
        ...
        if (sellerEscolhido.sellerId && sellerEscolhido.sellerId !== '1') {
            ...
            console.log('[assistente-ean] Drogaria São Paulo: MARKETPLACE detectado, vendedor', nomeVendedor, '- preco capturado:', precoMarketplace);
            encerrarAba();
            return;
        }
        ...
        if (!preco) {
            ...
            console.log('[assistente-ean] Drogaria São Paulo: preco INDISPONIVEL para', ean);
            encerrarAba();
            return;
        }
        ...
        if (porNome) {
            ...
            console.log('[assistente-ean] Drogaria São Paulo: preco POR NOME:', preco, 'EAN do site:', gtinItem);
            ...
            console.log('[assistente-ean] Drogaria São Paulo: EAN divergente. Buscado:', ean, 'API:', gtinItem);
        } else {
            emitirResultado(montarSentinel(ean, 'OK', preco, estoque, obs, nome));
            console.log('[assistente-ean] Drogaria São Paulo: preco copiado:', preco, 'Obs:', obs || '(sem promo)');
        }

        encerrarAba();
    }

    console.log('[assistente-ean] Drogaria São Paulo v2.2 ativo em', location.pathname,
    ...
    if (EAN_DO_FRAGMENTO) {
        ...
        setTimeout(() => {
            try {
                ...
                if (p && p.catch) {
                    p.catch((e) => {
                        console.error('[assistente-ean] Drogaria São Paulo: erro na consulta', e);
                        emitirResultado(montarSentinel(EAN_DO_FRAGMENTO, 'NAO_ENCONTRADO', '', '', 'erro interno do script', ''));
                        encerrarAba();
                    });
                }
            } catch (e) {
                console.error('[assistente-ean] Drogaria São Paulo: erro sincrono', e);
                emitirResultado(montarSentinel(EAN_DO_FRAGMENTO, 'NAO_ENCONTRADO', '', '', 'erro interno do script', ''));
                encerrarAba();
            }
        }, 800);
