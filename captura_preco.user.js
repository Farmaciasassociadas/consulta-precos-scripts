...
(function () {
    ...
    // ------------------------------------------------------------------
    // DICIONARIO DE TERMOS (marca/principio ativo/fabricante) - Fase 2.
    // Baixado do repositorio publico, cacheado ~12h (GM_setValue). Uso
    // SEMPRE ADITIVO na coerencia: termo desconhecido nunca bloqueia nada -
    // so reforca a rejeicao quando os DOIS lados tem resposta conhecida e
    // ela diverge (marca diferente, ou principio ativo diferente - direto
    // ou via marca). Qualquer falha (rede, parse, GM indisponivel) e
    // silenciosa: a coleta segue exatamente como sem o dicionario.
    // ------------------------------------------------------------------
    ...
    try {
        ...
        if ((!bruto || (Date.now() - quando) > DICIONARIO_CACHE_MS) && typeof GM_xmlhttpRequest === 'function') {
            GM_xmlhttpRequest({
                ...
                onload: (resp) => {
                    try {
                        JSON.parse(resp.responseText); // valida antes de cachear
                        GM_setValue('dicionario_termos_cache', resp.responseText);
                        GM_setValue('dicionario_termos_cache_hora', Date.now());
                    } catch (e) { /* resposta invalida: mantem cache antigo */ }
                },
                onerror: () => { }, ontimeout: () => { },
            });
        }
    } catch (e) { /* GM indisponivel: segue sem dicionario */ }
    ...
    function paginaDeBusca() {
        ...
        // Bloqueio de WAF (Akamai) na própria busca — página de erro simples
        // ("Access Denied" / edgesuite.net), sem SPA, sem card e sem o texto
    ...
    function paginaDeProduto() {
        ...
        if (vendedor) {
            ...
            console.log('[assistente-ean] Raia: MARKETPLACE detectado, vendedor', vendedor, '- preco capturado:', preco);
            encerrarAba();
            return;
        }
