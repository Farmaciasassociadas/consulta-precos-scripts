...
// @match        https://www.precopopular.com.br/*
...
(function () {
    ...
    // O site da Preço Popular roda em VTEX, que expõe uma API pública de catálogo.
    ...
    // "Principio Ativo": a Preço Popular e o site que mais expoe isso de forma
    ...
    // A Preço Popular guarda o EAN EXATAMENTE como cadastrado (com zeros à
    ...
    // A Preço Popular é VTEX com REGIONALIZAÇÃO: parte do catálogo (ex.: genéricos
    ...
    function emitirDeProduto(produto, ean, porNome) {
        ...
        if (sellerEscolhido.sellerId && sellerEscolhido.sellerId !== '1') {
            ...
            console.log('[assistente-ean] Preço Popular: MARKETPLACE detectado, vendedor', nomeVendedor, '- preco NAO capturado');
            encerrarAba();
            return;
        }
    ...
    if (EAN_DO_FRAGMENTO) {
        ...
        setTimeout(() => {
            try {
                ...
                if (p && p.catch) {
                    p.catch((e) => {
                        console.error('[assistente-ean] Preço Popular: erro na consulta', e);
                        emitirResultado(montarSentinel(EAN_DO_FRAGMENTO, 'NAO_ENCONTRADO', '', '', 'erro interno do script', ''));
                        encerrarAba();
                    });
                }
            } catch (e) {
                console.error('[assistente-ean] Preço Popular: erro sincrono', e);
                emitirResultado(montarSentinel(EAN_DO_FRAGMENTO, 'NAO_ENCONTRADO', '', '', 'erro interno do script', ''));
                encerrarAba();
            }
        }, 800);
