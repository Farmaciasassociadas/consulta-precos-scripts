...
// @match        https://www.paguemenos.com.br/*
...
(function () {
    ...
    const SITE = 'paguemenos';
    // BLOQUEIO ANTIBOT (07/2026): Pague Menos consulta via API/fetch (não navega
    ...
    // NAO_ENCONTRADO — não grava nada, item volta pendente, Pague Menos pausa
    ...
    // O site da Pague Menos roda em VTEX, que expõe uma API pública de catálogo.
    ...
    // "Principio Ativo": a Pague Menos e o site que mais expoe isso de forma
    ...
    // A Pague Menos guarda o EAN EXATAMENTE como cadastrado (com zeros à
    ...
    // A Pague Menos é VTEX com REGIONALIZAÇÃO: parte do catálogo (ex.: genéricos
    ...
    async function consultar() {
        ...
        if (!produto) {
            for (const termo of variacoesDoEan(ean)) {
                ...
                if (certo) {
                    produto = certo;
                    console.log('[assistente-ean] Pague Menos: achado pela Intelligent Search (regional)');
                    break;
                }
            }
        }
        ...
        if (!produto) {
            ...
            for (const l of links.slice(0, 3)) {
                ...
                if (eanBate(p)) {
                    p.urlPagina = l.url;
                    produto = p;
                    console.log('[assistente-ean] Pague Menos: achado pela PAGINA de busca (fallback DOM)');
                    break;
                }
            }
        }
        ...
        if (!produto) {
            if (bloqueioSuspeito) {
                ...
                console.log('[assistente-ean] Pague Menos: resposta suspeita (bloqueio antibot?) para', ean);
            } else {
                emitirResultado(montarSentinel(ean, 'NAO_ENCONTRADO', '', '', '', ''));
                console.log('[assistente-ean] Pague Menos: nao encontrado:', ean);
            }
            encerrarAba();
            return;
        }
    ...
    async function consultarPorNome(ean) {
        ...
        if (melhor && melhorNota >= LIMIAR_NOME) {
            console.log('[assistente-ean] Pague Menos: candidato por nome aceito (nota', melhorNota.toFixed(2), ')');
        ...
        if (melhorLink && notaLink >= LIMIAR_NOME) {
            ...
            if (produto) {
                ...
                console.log('[assistente-ean] Pague Menos: candidato por nome via PAGINA (nota', notaLink.toFixed(2), ')');
                emitirDeProduto(produto, ean, true);
                return;
            }
        }
        console.log('[assistente-ean] Pague Menos: nenhum candidato por nome (melhor:',
            Math.max(melhorNota, notaLink).toFixed(2), ')');
        if (bloqueioSuspeito) {
            emitirResultado(montarSentinel(ean, 'BLOQUEIO', '', '', '', ''));
            console.log('[assistente-ean] Pague Menos: resposta suspeita (bloqueio antibot?) na busca por nome para', ean);
        } else {
            emitirResultado(montarSentinel(ean, 'NAO_ENCONTRADO', '', '', '', ''));
        }
        encerrarAba();
    ...
    function emitirDeProduto(produto, ean, porNome) {
        ...
        if (sellerEscolhido.sellerId && sellerEscolhido.sellerId !== '1') {
            ...
            console.log('[assistente-ean] Pague Menos: MARKETPLACE detectado, vendedor', nomeVendedor, '- preco capturado:', precoMarketplace);
            encerrarAba();
            return;
        }
        ...
        if (!preco) {
            ...
            console.log('[assistente-ean] Pague Menos: preco INDISPONIVEL para', ean);
            encerrarAba();
            return;
        }
        ...
        if (porNome) {
            ...
            console.log('[assistente-ean] Pague Menos: preco POR NOME:', preco, 'EAN do site:', gtinItem);
            ...
            console.log('[assistente-ean] Pague Menos: EAN divergente. Buscado:', ean, 'API:', gtinItem);
        } else {
            emitirResultado(montarSentinel(ean, 'OK', preco, estoque, obs, nome));
            console.log('[assistente-ean] Pague Menos: preco copiado:', preco, 'Obs:', obs || '(sem promo)');
        }

        encerrarAba();
    }

    console.log('[assistente-ean] Pague Menos v2.2 ativo em', location.pathname,
    ...
    if (EAN_DO_FRAGMENTO) {
        ...
        setTimeout(() => {
            try {
                ...
                if (p && p.catch) {
                    p.catch((e) => {
                        console.error('[assistente-ean] Pague Menos: erro na consulta', e);
                        emitirResultado(montarSentinel(EAN_DO_FRAGMENTO, 'NAO_ENCONTRADO', '', '', 'erro interno do script', ''));
                        encerrarAba();
                    });
                }
            } catch (e) {
                console.error('[assistente-ean] Pague Menos: erro sincrono', e);
                emitirResultado(montarSentinel(EAN_DO_FRAGMENTO, 'NAO_ENCONTRADO', '', '', 'erro interno do script', ''));
                encerrarAba();
            }
        }, 800);
